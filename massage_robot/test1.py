...
def update_trajectory(human_body, traj_step, frequency, amp, x_offset, region, z_offset_lower, z_offset_upper):
    ...
    pnts = generate_trajectory(start, end, numSamples=traj_step, frequency=frequency, amp=amp)
    return np.vstack((pnts, pnts[::-1]))


def test_settings_single(physicsClient, human_inst, armId, nArmJoints, traj_step,
                         frequency, amp, x_offset, z_offset_lower, z_offset_upper, region, force):
    params = {'frequency': frequency, 'amp': amp, 'x_offset': x_offset,
              'z_offset_lower': z_offset_lower, 'z_offset_upper': z_offset_upper, 'region': region, 'force': force}
    print(f"Testing frequency={frequency}, amplitude={amp}, x_offset={x_offset}, "
          f"z_offset_lower={z_offset_lower}, z_offset_upper={z_offset_upper}, region={region}, force={force}")
    pntsAndReturn = update_trajectory(human_inst.body, traj_step, frequency, amp,
                                      x_offset, region, z_offset_lower, z_offset_upper)
...
def report_results(results):
    for res in results:
        ...
        print(f"Frequency: {params['frequency']}, Amplitude: {params['amp']}, X Offset: {params['x_offset']}, "
              ...
              f"Region: {params['region']}, Force: {params['force']}, Mean Force: {mean_force:.2f}")
    if results:
        last = results[-1]
        draw_data(last['Forces'], last['armparts'], last['bodyparts'])


def run_simulation_with_params(frequency, amplitude, x_offset, z_offset_lower, z_offset_upper, region, force):
    ...
    results = test_settings_single(physicsClient, human_inst, armId, nArmJoints, traj_step,
                                   frequency, amplitude, x_offset, z_offset_lower, z_offset_upper, region, force)
...
def start_simulation():
    ...
    threading.Thread(target=lambda: [run_simulation_with_params(freq, amp, x_offset,
                                                               z_offset_lower, z_offset_upper, region, force),
