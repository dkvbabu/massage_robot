...
# Original functions from test_viewer.py
...
# New trajectory generation functions
...
def run_simulation_with_params(frequency, amplitude, x_offset, z_offset_lower, z_offset_upper, region, force, traj_type, massage_technique):
    ...
    camera_thread = threading.Thread(target=camera_control, daemon=True)
    camera_thread.start()
...
def start_simulation():
    try:
        ...
        # Validate force limit
        if force > 200:
            messagebox.showerror("Invalid Force", "Force cannot exceed 200")
            return
...
def update_camera_yaw(value):
    global camera_yaw
    camera_yaw = float(value)
    update_camera()

def update_camera_pitch(value):
    global camera_pitch
    camera_pitch = float(value)
    update_camera()

def update_camera_distance(value):
    global camera_distance
    camera_distance = float(value)
    update_camera()
...
sim_frame = ttk.LabelFrame(main_frame, text="Simulation Parameters", padding="5")
sim_frame.grid(row=0, column=0, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S))

tk.Label(sim_frame, text="Frequency:").grid(row=0, column=0, padx=5, pady=5)
freq_entry = tk.Entry(sim_frame)
freq_entry.insert(0, "6")
freq_entry.grid(row=0, column=1, padx=5, pady=5)

tk.Label(sim_frame, text="Amplitude:").grid(row=1, column=0, padx=5, pady=5)
amp_entry = tk.Entry(sim_frame)
amp_entry.insert(0, "0.035")
amp_entry.grid(row=1, column=1, padx=5, pady=5)

tk.Label(sim_frame, text="X Offset:").grid(row=2, column=0, padx=5, pady=5)
x_offset_entry = tk.Entry(sim_frame)
x_offset_entry.insert(0, "0.1")
x_offset_entry.grid(row=2, column=1, padx=5, pady=5)

tk.Label(sim_frame, text="Z Offset Lower:").grid(row=3, column=0, padx=5, pady=5)
z_offset_lower_entry = tk.Entry(sim_frame)
z_offset_lower_entry.insert(0, "0.01")
z_offset_lower_entry.grid(row=3, column=1, padx=5, pady=5)

tk.Label(sim_frame, text="Z Offset Upper:").grid(row=4, column=0, padx=5, pady=5)
z_offset_upper_entry = tk.Entry(sim_frame)
...
tk.Label(sim_frame, text="Target Region:").grid(row=5, column=0, padx=5, pady=5)
region_menu = tk.OptionMenu(sim_frame, region_var, 'lower_back', 'upper_back')
region_menu.grid(row=5, column=1, padx=5, pady=5)

tk.Label(sim_frame, text="Force (max 200):").grid(row=6, column=0, padx=5, pady=5)
force_entry = tk.Entry(sim_frame)
force_entry.insert(0, "100")
force_entry.grid(row=6, column=1, padx=5, pady=5)

traj_type_var = tk.StringVar(value='sine')
tk.Label(sim_frame, text="Trajectory Type:").grid(row=7, column=0, padx=5, pady=5)
traj_type_menu = tk.OptionMenu(sim_frame, traj_type_var, 'sine', 'circular', 'linear')
traj_type_menu.grid(row=7, column=1, padx=5, pady=5)

massage_technique_var = tk.StringVar(value='normal')
tk.Label(sim_frame, text="Massage Technique:").grid(row=8, column=0, padx=5, pady=5)
massage_technique_menu = tk.OptionMenu(sim_frame, massage_technique_var, 'normal', 'kneading', 'pressure')
...
camera_frame.grid(row=0, column=1, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S))

tk.Label(camera_frame, text="Yaw (0-360):").grid(row=0, column=0, padx=5, pady=5)
yaw_scale = ttk.Scale(camera_frame, from_=0, to=360, orient=tk.HORIZONTAL, command=update_camera_yaw)
yaw_scale.set(0)
yaw_scale.grid(row=0, column=1, padx=5, pady=5)

tk.Label(camera_frame, text="Pitch (-90-90):").grid(row=1, column=0, padx=5, pady=5)
pitch_scale = ttk.Scale(camera_frame, from_=-90, to=90, orient=tk.HORIZONTAL, command=update_camera_pitch)
pitch_scale.set(-30)
pitch_scale.grid(row=1, column=1, padx=5, pady=5)

tk.Label(camera_frame, text="Distance (0.5-5):").grid(row=2, column=0, padx=5, pady=5)
distance_scale = ttk.Scale(camera_frame, from_=0.5, to=5.0, orient=tk.HORIZONTAL, command=update_camera_distance)
distance_scale.set(1.0)
distance_scale.grid(row=2, column=1, padx=5, pady=5)

# Start Button
start_button = tk.Button(main_frame, text="Start Simulation", command=start_simulation)
