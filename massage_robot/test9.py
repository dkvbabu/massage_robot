...
def visualize_contact(armId, human_body, physicsClient):
    """
    Visualizes contact by changing the color of the contacting link.
    """
    contact_points = p.getContactPoints(bodyA=armId, bodyB=human_body, physicsClientId=physicsClient)
    if contact_points:
        # Get the link index of the contacting link on the robot arm
        contacting_link_index = contact_points[0][3]  # Link index on bodyA (armId)

        # Change the color of the contacting link to red
        p.changeVisualShape(objectUniqueId=armId, linkIndex=contacting_link_index, rgbaColor=[1, 0, 0, 1], physicsClientId=physicsClient)
    else:
        # Reset the color of all links on the robot arm to default (e.g., white)
...
def visualize_contact_points(armId, human_body, physicsClient):
    ...
    for point in contact_points:
        position_on_A = point[5]  # Contact position on body A (arm)
        position_on_B = point[6]  # Contact position on body B (human)

        # Draw a line between the contact points
...
def visualize_contact_force(armId, human_body, physicsClient):
    contact_points = p.getContactPoints(bodyA=armId, bodyB=human_body, physicsClientId=physicsClient)
    if contact_points:
        # Get the link index and contact force
        contacting_link_index = contact_points[0][3]
        contact_force = contact_points[0][9]

        # Normalize the contact force to a color intensity (0 to 1)
        max_force = 100  # Adjust this based on your expected force range
        intensity = min(contact_force / max_force, 1.0)

        # Set the color based on the intensity (e.g., red to white)
        color = [1, 1 - intensity, 1 - intensity, 1]  # Red to white
...
class MassageEnv:
    def __init__(self, physicsClient, armId, human_inst, nArmJoints, traj_step=100,
        ...
        # Initialize visualization handles BEFORE calling _update_trajectory
        ...
        print("Before configureDebugVisualizer")
        p.configureDebugVisualizer(10, 1, physicsClientId=self.physicsClient)
        print("After configureDebugVisualizer")
    def reset(self):
    ...
    def _update_trajectory(self):
        """
        Generate trajectory points with hardcoded y-offset based on region,
        and visualize the trajectory points in PyBullet GUI.
        """
        p1, p2 = p.getAABB(self.human_inst.body)

        # Hardcoded y-offset based on region
        if self.region == 'lower_back':
            z_pos = p1[2] + self.z_offset_lower
            y_pos = 0.03  # Hardcoded y-offset for lower_back
        elif self.region == 'upper_back':
            z_pos = p2[2] - self.z_offset_upper
            y_pos = 0.4  # Hardcoded y-offset for upper_back
        else:
            z_pos = (p1[2] + p2[2]) / 2
            y_pos = 0.3  # default y-offset

        # Define start, end, and center points for trajectory generation
        start = np.array([(p1[0] + p2[0]) / 2 - self.x_offset, y_pos, z_pos])
        end = np.array([(p1[0] + p2[0]) / 2 + self.x_offset, y_pos, z_pos])
        center = np.array([(p1[0] + p2[0]) / 2, y_pos, z_pos])

        # Generate trajectory points based on massage technique
        if self.massage_technique == 'kneading':
            radius = self.x_offset / 2  # smaller radius for kneading
        ...
        # Combine forward and reverse trajectory for smooth looping
        trajectory_points = np.vstack((pnts, pnts[::-1]))
        self.trajectory_points = trajectory_points

        # Visualization: Remove old trajectory visuals safely
        for vis_id in self.trajectory_vis_ids:
            try:
                # Check if the body ID is still valid
                p.getBodyInfo(vis_id, physicsClientId=self.physicsClient)
                p.removeBody(vis_id, physicsClientId=self.physicsClient)
            except p.error:
                # Body does not exist, skip removal
                #print(f"Warning: Body ID {vis_id} does not exist, skipping removal.")
                continue
            except Exception as e:
                # Handle other exceptions
                #print(f"An unexpected error occurred while removing body {vis_id}: {e}")
                continue

        self.trajectory_vis_ids = []  # Clear the list after attempting to remove bodies

        # Draw new trajectory points as green spheres
        for point in trajectory_points:
            sphere_radius = 0.01
            sphere_color = [0, 1, 0, 1]  # Green RGBA
    ...
    def step(self, action_idx):
        ...
        # --- Safe removal of previous end-effector visualization ---
        if self.ee_vis_id is not None:
            try:
                ...
                #print(f"Warning: End-effector body ID {self.ee_vis_id} does not exist, skipping removal.")
                pass
            except Exception as e:
                #print(f"An unexpected error occurred while removing end-effector body {self.ee_vis_id}: {e}")
                pass

        # Draw new end-effector visualization sphere
        ee_link_state = p.getLinkState(self.armId, self.nArmJoints - 2, physicsClientId=self.physicsClient)
        ee_position = ee_link_state[0]  # world position

        sphere_radius = 0.02
        sphere_color = [1, 0, 0, 1]  # Red RGBA
        sphere_vis_id = p.createVisualShape(shapeType=p.GEOM_SPHERE, rgbaColor=sphere_color, radius=sphere_radius)
...
def run_rl_training(frequency, amplitude, x_offset, z_offset_lower, z_offset_upper,
                    ...
                    hyperparams=None, load_model=False):
    ...
    # Recreate optimizer if learning rate changed
    ...
    def run_rl():
        try:
            ...
            run_rl_training(freq, amp, x_offset, z_offset_lower, z_offset_upper,
                            region, force, traj_type, massage_technique,
                            max_rl_episodes=200, save_dir='models/dqn_experiment1',
    ...
    def run_ppo():
        try:
            ...
            run_ppo_training(freq, amp, x_offset, z_offset_lower, z_offset_upper,
                            ...
                            save_dir='models/ppo_experiment1', save_interval=10,
