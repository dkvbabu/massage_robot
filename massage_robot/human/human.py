...
class Human(Agent):
    ...
    def init(self, human_creation, limits_model, static_human_base, impairment, gender, config, id, np_random, mass=None, radius_scale=1.0, height_scale=1.0):
        ...
        self.base = self.body  # Add this line to set base ID
    ...
    def setup_joints2(self, joints_positions, use_static_joints=True, reactive_force=None, reactive_gain=0.05):
        # Set static joints
        #joint_angles = self.get_joint_angles_dict(self.all_joint_indices)
        for j in self.all_joint_indices:
            if use_static_joints and (j not in self.controllable_joint_indices or (self.impairment != 'tremor' and reactive_force is None and not self.controllable)):
                # Make all non controllable joints on the person static by setting mass of each link (joint) to 0
                p.changeDynamics(self.body, j, mass=0.0,lateralFriction=2, physicsClientId=self.id)
                #p.changeDynamics(self.body, j, mass=10,lateralFriction=2, restitution = 0.7,jointLimitForce=10, physicsClientId=self.id)
                # Set velocities to 0
                #self.set_joint_angles([j], [0])#joint_angles[j]

        # Set starting joint positions
        #self.set_joint_angles([j for j, _ in joints_positions], [0 for _, j_angle in joints_positions])
