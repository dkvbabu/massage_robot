...
class MassageEnv():
    ...
    def __init__(self,render=False,auto_reset=True):
        ...
        # Debug print all joints and links
        print("Number of joints:", self.nArmJoints)
        for i in range(self.nArmJoints):
            info = p.getJointInfo(self.armId, i)
            print(f"Joint {i}: name={info[1].decode('utf-8')}, link_name={info[12].decode('utf-8')}")

        # Temporarily set EndEfferctorId to 6 (adjust after checking print output)
        self.EndEfferctorId = 7
    ...
    def get_contact_points(self):
        ...
        out_1 = p.getContactPoints(bodyA=self.armId, bodyB=self.human_inst.body,
                                  linkIndexA=self.EndEfferctorId, physicsClientId=self.SimID)
    ...
    def get_reward(self,stats):
        ...
        #print('wrongcontact',WrongContact)
        # print('highpressure',HighPressure)
        # print('force', Force)
        # print(f"Contact link index: {armpart}, End Effector ID: {self.EndEfferctorId}")
        # print(f"WrongContact: {WrongContact}")
        # print('reward',reward)
