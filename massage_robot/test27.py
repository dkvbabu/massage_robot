...
def update_ball_position_contact(physicsClient, sphereId, humanBodyId, radius, step_size=0.001, max_steps=50):
    # Get current ball position and orientation
    ball_pos, ball_orn = p.getBasePositionAndOrientation(sphereId, physicsClientId=physicsClient)

    # Check if ball is already in contact with human body
    contacts = p.getContactPoints(bodyA=sphereId, bodyB=humanBodyId, physicsClientId=physicsClient)

    if contacts:
        # Ball is already touching the body, maintain position
        return

    # If no contact, lower the ball gradually until contact or max steps reached
    for i in range(max_steps):
        new_z = ball_pos[2] - step_size
        new_pos = (ball_pos[0], ball_pos[1], new_z)
        p.resetBasePositionAndOrientation(sphereId, new_pos, ball_orn, physicsClientId=physicsClient)

        contacts = p.getContactPoints(bodyA=sphereId, bodyB=humanBodyId, physicsClientId=physicsClient)
        if contacts:
            print(f"Contact established after lowering {step_size * (i+1):.4f} meters")
            break

        ball_pos = new_pos  # update for next iteration

def update_ball_position(physicsClient, armId, end_effector_link_index, sphereId, humanBodyId, radius, smooth_factor=0.1):
    ee_state = p.getLinkState(armId, end_effector_link_index, physicsClientId=physicsClient)
    ee_pos = ee_state[0]
    ee_orn = ee_state[1]

    # Smooth the end effector position
    if not hasattr(update_ball_position, "prev_ee_pos"):
        update_ball_position.prev_ee_pos = ee_pos
    ee_pos = [smooth_factor * ee_pos[i] + (1 - smooth_factor) * update_ball_position.prev_ee_pos[i] for i in range(3)]
    update_ball_position.prev_ee_pos = ee_pos

    # Adjust raycast start and end points
    ray_start = [ee_pos[0], ee_pos[1], ee_pos[2] + 0.1]  # Start slightly above
    ray_end = [ee_pos[0], ee_pos[1], ee_pos[2] - 0.5]  # Extend further down

    # Debug line
    p.addUserDebugLine(ray_start, ray_end, [0, 1, 0], 3, lifeTime=0.1)

    ray_result = p.rayTest(ray_start, ray_end, physicsClientId=physicsClient)
    hit_object_uid = ray_result[0][0]
    hit_position = ray_result[0][3]

    margin = 0.01  # 1 cm margin

    if hit_object_uid == humanBodyId:
        new_z = hit_position[2] + radius + margin
    else:
        new_z = ee_pos[2]

    new_pos = [ee_pos[0], ee_pos[1], new_z]

    p.resetBasePositionAndOrientation(sphereId, new_pos, ee_orn, physicsClientId=physicsClient)

    
def keep_ball_touching_body(physicsClient, sphereId, humanBodyId, radius):
    # Get current ball position
    ball_pos, ball_orn = p.getBasePositionAndOrientation(sphereId, physicsClientId=physicsClient)

    # Cast ray downward from ball x,y at a height above expected max body height
    ray_start = [ball_pos[0], ball_pos[1], ball_pos[2] + 0.2]
    ray_end = [ball_pos[0], ball_pos[1], ball_pos[2] - 0.5]
    ...
    if hit_object_uid == humanBodyId:
        # Set new ball z position so ball just touches body surface
        new_z = hit_position[2] + radius

        # Update ball position (keep x,y same)
        new_pos = [ball_pos[0], ball_pos[1], new_z]

        p.resetBasePositionAndOrientation(sphereId, new_pos, ball_orn, physicsClientId=physicsClient)


def draw_link_frame(physicsClient, armId, linkIndex, length=0.1):
    link_state = p.getLinkState(armId, linkIndex, physicsClientId=physicsClient)
    pos, orn = link_state[0], link_state[1]
    rot_matrix = p.getMatrixFromQuaternion(orn)
    rot_matrix = [rot_matrix[0:3], rot_matrix[3:6], rot_matrix[6:9]]

    # X axis (red)
    x_axis = [pos[0] + length * rot_matrix[0][0],
              pos[1] + length * rot_matrix[1][0],
              pos[2] + length * rot_matrix[2][0]]
    p.addUserDebugLine(pos, x_axis, [1, 0, 0], 2, 0.1, physicsClientId=physicsClient)

    # Y axis (green)
    y_axis = [pos[0] + length * rot_matrix[0][1],
              pos[1] + length * rot_matrix[1][1],
              pos[2] + length * rot_matrix[2][1]]
    p.addUserDebugLine(pos, y_axis, [0, 1, 0], 2, 0.1, physicsClientId=physicsClient)

    # Z axis (blue)
    z_axis = [pos[0] + length * rot_matrix[0][2],
              pos[1] + length * rot_matrix[1][2],
              pos[2] + length * rot_matrix[2][2]]
    p.addUserDebugLine(pos, z_axis, [0, 0, 1], 2, 0.1, physicsClientId=physicsClient)

import numpy as np
import pybullet as p
from pybullet import getMatrixFromQuaternion

def add_ball_touch_only(physicsClient, armId, end_effector_link_index, bodyId, radius=0.025, mass=1.0, offset=-0.025):
    import numpy as np
    from pybullet import getMatrixFromQuaternion
    ...
    # Create collision and visual shape for the ball
    colSphereId = p.createCollisionShape(p.GEOM_SPHERE, radius=radius, physicsClientId=physicsClient)
    visSphereId = p.createVisualShape(p.GEOM_SPHERE, radius=radius, rgbaColor=[1, 0, 0, 1], physicsClientId=physicsClient)

    # Compute offset in world frame
    local_offset = [0, 0, offset]
    rot_matrix = np.array(getMatrixFromQuaternion(ee_orn)).reshape(3, 3)
    world_offset = rot_matrix.dot(local_offset)
    ball_pos = [ee_pos[0] + world_offset[0], ee_pos[1] + world_offset[1], ee_pos[2] + world_offset[2]]

    # Create the ball rigid body
    ...
    # Set friction and restitution for rolling behavior
    p.changeDynamics(sphereId, -1, lateralFriction=0.8, restitution=0.5,
                     rollingFriction=0.01, spinningFriction=0.01, physicsClientId=physicsClient)

    # Attach ball to end effector with fixed constraint
    cid = p.createConstraint(parentBodyUniqueId=armId,
                             parentLinkIndex=end_effector_link_index,
                             childBodyUniqueId=sphereId,
                             childLinkIndex=-1,
                             jointType=p.JOINT_FIXED,
                             jointAxis=[0, 0, 0],
                             parentFramePosition=local_offset,
                             parentFrameOrientation=[0, 0, 0, 1],
                             childFramePosition=[0, 0, 0],
                             childFrameOrientation=[0, 0, 0, 1],
                             physicsClientId=physicsClient)

    # Disable collisions between all arm links and the body except the ball
    nJoints = p.getNumJoints(armId)
    for link_idx in range(-1, nJoints):  # -1 for base link
        if link_idx == end_effector_link_index:
            continue  # keep collision enabled for end effector link if needed
        p.setCollisionFilterPair(armId, bodyId, link_idx, -1, enableCollision=0, physicsClientId=physicsClient)

    # Disable collisions between arm base (-1) and body as well
    p.setCollisionFilterPair(armId, bodyId, -1, -1, enableCollision=0, physicsClientId=physicsClient)
...
def update_trajectory(human_body, traj_step, frequency, amp, x_offset, region, z_offset_lower, z_offset_upper, traj_type, massage_technique):
    ...
    inward_offset_y = 0.02  # 2 cm inward on Y axis
    inward_offset_x = 0.1  # 5 cm inward on X axis (adjust as needed)
    clearance_z = 0.15  # 1.5 cm above the surface
    ...
    # Apply clearance offset for pressure massage to avoid piercing
    if region == 'upper_back':
        z_pos += clearance_z 

    # Adjust start and end X positions inward by inward_offset_x
    ...
    if region == 'upper_back':
        print(f"Human upper back AABB z: {p2[2]}, y_pos before offset: 0.4")
        print(f"Adjusted y_pos: {y_pos}")
        print(f"Start point: {start}, End point: {end}, Center: {center}")
...
def test_settings_single(physicsClient, human_inst, armId, nArmJoints, traj_step,
                         frequency, amp, x_offset, z_offset_lower, z_offset_upper, region, force, traj_type, massage_technique,sphereId,humanBodyId,end_effector_link_index):
    ...
    for j in range(1200):
        try:
            ...
            # Check contact between ee_link and human back
            ...
            # Draw the end effector coordinate frame for debugging
            #draw_link_frame(physicsClient, armId, 8)
            # Keep ball touching body surface
            
            #update_ball_position_contact(physicsClient, sphereId, humanBodyId, radius=0.025)
            #update_ball_position(physicsClient, armId, end_effector_link_index, sphereId, humanBodyId, radius=0.025)
            ...
            #JointPoses = list(p.calculateInverseKinematics(armId, nArmJoints - 1, pntsAndReturn[j % (2 * traj_step)]))
            upper_back_orientation = p.getQuaternionFromEuler([np.pi, 0, np.pi/2])
            if region == 'lower_back':
                orientation = p.getQuaternionFromEuler([0, np.pi, 0])  # existing down_orientation
            elif region == 'upper_back':
                orientation = upper_back_orientation  # try one of the above
            else:
                orientation = p.getQuaternionFromEuler([0, np.pi, 0])  # default
...
def run_simulation_with_params(frequency, amplitude, x_offset, z_offset_lower, z_offset_upper, region, force, traj_type, massage_technique):
    ...
    #debug_ee_link_position(physicsClient, armId, ee_link_index, duration=5, interval=0.2)
    humanBodyId = human_inst.base  # from your human model instance
    print("Human body ID:", humanBodyId)
    print('eelinkindex',ee_link_index)

   
    # === Collision Filtering Setup ===
    # Disable collisions between robot arm and plane
    num_joints_arm = p.getNumJoints(armId, physicsClientId=physicsClient)
    for joint_idx in range(-1, num_joints_arm):  # -1 is base
    ...
    # Disable collisions between human and plane
    ...
    # # Add the ball (sphere) and disable its collisions with plane and arm
    # sphereId, constraintId = add_ball_touch_only(physicsClient, armId, end_effector_link_index, humanBodyId,
    #                                              radius=0.025, mass=1.0, offset=-0.025)
    #  # List all bodies and their names
    # print("Listing all bodies in the simulation:")
    # print('sphereid',sphereId)
    # for body_id in range(p.getNumBodies()):
    #     info = p.getBodyInfo(body_id)
    #     print(f"Body ID: {body_id}, Name: {info[1].decode('utf-8')}")
    ...
    for joint_idx in range(-1, num_joints_arm):
        p.setCollisionFilterPair(armId, planeId, joint_idx, -1, 0, physicsClientId=physicsClient)
        p.setCollisionFilterPair(armId, humanBodyId, joint_idx, -1, 0, physicsClientId=physicsClient)
        p.setCollisionFilterPair(armId, bedId, joint_idx, -1, 0, physicsClientId=physicsClient)

    # # Disable collisions between sphere and plane, arm, bed
    # p.setCollisionFilterPair(sphereId, planeId, -1, -1, 0, physicsClientId=physicsClient)
    # p.setCollisionFilterPair(sphereId, armId, -1, -1, 0, physicsClientId=physicsClient)
    # p.setCollisionFilterPair(sphereId, bedId, -1, -1, 0, physicsClientId=physicsClient)

    # Disable collisions between human and plane, bed
    num_joints_human = p.getNumJoints(humanBodyId, physicsClientId=physicsClient)
    for joint_idx in range(-1, num_joints_human):
        p.setCollisionFilterPair(humanBodyId, planeId, joint_idx, -1, 0, physicsClientId=physicsClient)
        p.setCollisionFilterPair(humanBodyId, bedId, joint_idx, -1, 0, physicsClientId=physicsClient)
    # Check for any initial collisions between arm and human
...
# Validation function for force input
def validate_force_input(new_value):
    if new_value == "":
        return True  # allow empty string while typing
    try:
        val = float(new_value)
        return 0 <= val <= 100
    except ValueError:
...
# Modify force_entry to use validation
