...
def run_inference_with_trained_model(model_type='dqn', model_path=None, num_steps=500):
    ...
    if model_type.lower() == 'dqn':
        ...
        # Use the same hidden_size as used during training (256 in your case)
        agent = PPOAgent(state_size, action_size, device=device, hidden_size=256)
        if model_path is not None:
            agent.load(model_path)
    else:
...
def start_dqn_inference_gui():
    ...
    def run_infer():
        try:
            model_path = 'models/dqn_experiment1/dqn_model.pth'
...
def run_td3_training(frequency, amplitude, x_offset, z_offset_lower, z_offset_upper,
    ...
    state_size = len(env._get_state())
    action_size = len(env.joint_indices)  # continuous action per joint
    max_action = env.delta_angle  # max increment per joint
...
def start_ppo_inference_gui():
    ...
    def run_infer():
        try:
            model_path = 'models/ppo_experiment1/ppo_model.pth'
    ...
    def run_rl():
        try:
            ...
            run_rl_training(freq, amp, x_offset, z_offset_lower, z_offset_upper,
                            region, force, traj_type, massage_technique,
                            max_rl_episodes=400, save_dir='models/dqn_experiment1',
...
# Add buttons to your GUI window (assuming root is your Tk window)
train_td3_button = tk.Button(root, text="Train TD3", command=start_td3_training_gui)
train_td3_button.grid(row=10, column=0, padx=10, pady=10)

infer_td3_button = tk.Button(root, text="Run TD3 Inference", command=start_td3_inference_gui)
infer_td3_button.grid(row=10, column=1, padx=10, pady=10)
